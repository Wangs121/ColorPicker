...
class AskColor(customtkinter.CTkToplevel):
    ...
    def set_initial_color(self, initial_color):
        ...
        print(f"set init color{initial_color}")
            ...
            for i in range(0, self.image_dimension):
                for j in range(0, self.image_dimension):
                    ...
                    if (self.rgb_color[0], self.rgb_color[1], self.rgb_color[2])==(r,g,b):
                        ...
                        self.target_y = j
                        return
            self.set_target_position(r, g, b)
    ...
    def set_target_position(self, r, g, b):
        ...
        print(f"r: {r}, g: {g}, Value: {b}")
        h, s, v = colorsys.rgb_to_hsv(e / 255.0, g / 255.0, b / 255.0)
        print(f"Hue: {h * 360}, Saturation: {s}, Value: {v}")

        # 计算色调对应的角度（从0°到360°）
        theta = h * 360  # 色调h已经是0到1之间的值

        # 将角度转为弧度
        theta_rad = math.radians(theta)

        # 计算色环上的x和y坐标
        x = self.image_dimension / 2 * math.cos(theta_rad)
        y = self.image_dimension / 2 * math.sin(theta_rad)

        print(f"Position on color ring: ({x}, {y})")
